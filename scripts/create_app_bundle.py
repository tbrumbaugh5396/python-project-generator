#!/usr/bin/env python3
"""
Create macOS Application Bundle for Python Project Generator
...
def create_app_bundle():
	...
	app_name = "Python Project Generator"
	...
	# - Include the source package so we can import `python_project_generator`
	root = Path.cwd()
	src_dir = root / "src"
	if src_dir.exists():
		# Copy the entire src tree (contains `python_project_generator` package)
...
# Ensure we can import the `python_project_generator` package from Resources/src
...
try:
    # Prefer running the package entrypoint (GUI by default)
    from python_project_generator.__main__ import main as run_main
    run_main()
...
"""
	...
	plist_data = {
		...
		'CFBundleIdentifier': 'com.pythonprojectgenerator.app',
		'CFBundleVersion': '1.0.0',
		'CFBundleShortVersionString': '1.0.0',
		'CFBundleExecutable': app_name.replace(" ", ""),
		'CFBundleIconFile': 'ppg_icon.icns',
...
def create_icns_icon(resources_path):
	...
	try:
		...
		iconset_dir = resources_path / "ppg_icon.iconset"
		iconset_dir.mkdir(exist_ok=True)

		# Icon size mappings for macOS (source files created by create_icon.py)
		icon_mappings = {
			'icon_16x16.png': 'ppg_icon_16x16.png',
			'icon_16x16@2x.png': 'ppg_icon_32x32.png',
			'icon_32x32.png': 'ppg_icon_32x32.png',
			'icon_32x32@2x.png': 'ppg_icon_64x64.png',
			'icon_128x128.png': 'ppg_icon_128x128.png',
			'icon_128x128@2x.png': 'ppg_icon_256x256.png',
			'icon_256x256.png': 'ppg_icon_256x256.png',
			'icon_256x256@2x.png': 'ppg_icon_512x512.png',
			'icon_512x512.png': 'ppg_icon_512x512.png',
			'icon_512x512@2x.png': 'ppg_icon_1024x1024.png'
		}
		...
		icns_path = resources_path / "ppg_icon.icns"
		...
		if result.returncode == 0:
			...
			largest_icon = icons_dir / "ppg_icon_512x512.png"
			if largest_icon.exists():
				shutil.copy2(largest_icon, resources_path / "ppg_icon.icns")

	except Exception as e:
		print(f"⚠️  Could not create ICNS icon: {e}")
		# Copy any available icon as fallback
		if (icons_dir / "ppg_icon.png").exists():
			shutil.copy2(icons_dir / "ppg_icon.png", resources_path / "ppg_icon.icns")
